...
- 2026-08-22：审计写入合并（500ms 延迟落盘窗口，批量执行 N 次全量写 → 1 次）。
- 2026-08-22：图标加载失败缓存 + 进行中去重 + LruCache 改 count-based 256 项（内存可控）。
- 2026-08-22：`getHistory()` 内存缓存 TTL 60s（避免重复执行慢速 `dumpsys appops`）。
- 2026-08-22：`ProcessRunner` 输出上限 256KB → 4MB（修复 `dumpsys appops` 大输出截断导致的记录不全）。

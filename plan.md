...
- 整体主题：还原原版「初音绿」静态色板（浅/深两套），禁用动态取色。
- 全部页面按 MIUI 风格一次性补全：应用列表、应用详情、批量、历史、设置、Root 引导，统一大圆角卡片、StatusChip 语义色、可收缩标题 CollapsingTitle、悬浮式底部导航与详情页毛玻璃顶栏。
- 仅使用 `MaterialTheme.colorScheme` 令牌，页面代码零硬编码颜色。
